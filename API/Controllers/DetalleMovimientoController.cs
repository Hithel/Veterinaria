...
// [Authorize]
    ...
    public class DetalleMovimientoController : ApiBaseController
    {
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<Pager<DetalleMovimientoDto>>> GetPaginacion([FromQuery] Params citaParams)
        {
            var entidad = await unitofwork.DetalleMovimientos.GetAllAsync(citaParams.PageIndex, citaParams.PageSize, citaParams.Search);
            var listEntidad = mapper.Map<List<DetalleMovimientoDto>>(entidad.registros);
            return new Pager<DetalleMovimientoDto>(listEntidad, entidad.totalRegistros, citaParams.PageIndex, citaParams.PageSize, citaParams.Search);
        }
