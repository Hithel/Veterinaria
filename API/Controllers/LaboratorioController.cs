...
// [Authorize]
    ...
    public class LaboratorioController : ApiBaseController
    {
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<Pager<LaboratorioDto>>> GetPaginacion([FromQuery] Params citaParams)
        {
            var entidad = await unitofwork.Laboratorios.GetAllAsync(citaParams.PageIndex, citaParams.PageSize, citaParams.Search);
            var listEntidad = mapper.Map<List<LaboratorioDto>>(entidad.registros);
            return new Pager<LaboratorioDto>(listEntidad, entidad.totalRegistros, citaParams.PageIndex, citaParams.PageSize, citaParams.Search);
        }
